...
export default {
  ...
  char26() {
    ...
    for (let i = 0; i < 26; i++) {
      str +=
        String.fromCharCode(ch_small.charCodeAt(0) + i) +
        String.fromCharCode(ch_big.charCodeAt(0) + i);
    }
    return str;
  },
  log(message) {
    ...
    this.folderVerify(this.address, () => {
      ...
      this.createFolder(foldersMap, i, () => {
        ...
        for (let file in this.files) {
          ...
          if (file === "__map__") {
            fs.writeFile(this.files[file], JSON.stringify(this.files), err => {
  ...
  getMap(type) {
    ...
    try {
      return JSON.parse(
        fs.readFileSync(this.address + productName + "/__map__.json")
      );
    } catch (e) {
      return false;
    }
