export default {
  ...
  start(txt, callback, cb) {
    ...
    for (let i = 0; i < txt.length; i++) {
      ...
      while ((item_time = this.regex_time.exec(item[1]))) {
        this.list.push([
          parseFloat(item_time[1]) * 60 + parseFloat(item_time[2]),
          item[2]
        ]);
      }
  ...
  jump(duration, cb) {
    ...
    while (left <= pivot && pivot <= right) {
      if (
        this.list[pivot][0] <= duration &&
        (pivot === right || duration < this.list[pivot + 1][0])
      ) {
    ...
    if (scrollList[pivot]) {
      document.getElementById(this.hoverPrefixId).innerHTML =
        scrollList[pivot].innerHTML;
      for (let i = 0; i < this.prefixIdHandle.length; i++) {
        this.prefixIdHandle[i].childNodes[
          pivot
        ].className = this.hoverClass.split(",")[i];
      }
      tmp =
        scrollList[pivot].offsetTop -
        scrollList[pivot].parentNode.offsetTop -
        this.hoverTop -
        30;
