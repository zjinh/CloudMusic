...
Vue.defaultAlbum = Vue.prototype.$defaultAlbum =
  "http://p1.music.126.net/oCnACmhB6CM5oZyWmNfmTg==/109951163051142326.jpg";
...
Vue.handleListData = Vue.prototype.$handleListData = (data, parent, flag) => {
  ...
  data.forEach((item, index) => {
    ...
    let time =
      item.duration || (item.mMusic ? item.mMusic.playTime : 0) || item.dt;
    let artist_text = "";
    artist.forEach((item, index) => {
      artist_text =
        artist_text + item.name + (index !== artist.length - 1 ? "/" : "");
    });
    list = {
      id: item.id,
      playList: parent,
