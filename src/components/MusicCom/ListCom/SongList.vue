	...
	<div class="cm-songlist-container">
		...
		>
			<li @click="clickToPlay(item, index)" :class="item.play + (index % 2 === 0 ? ' cm-list-odd' : '')" ripple="">
				...
				<span class="num sf-icon-volume-up" v-else />
				<span class="name">
					{{ item.title }}
					<button class="sf-icon-youtube-play" @click.stop="playMv(item)" v-if="item.mvid" />
				</span>
				<span class="singer">
					{{ item.artist }}
				</span>
				...
				<span class="control">
					<LikeMusic :music="item" :index="index" @like="likeCallback" />
					<AddMusic :music="item" />
					<DownMusic :music="item" />
					<button
						class="sf-icon-trash-alt"
						@click.stop="removeMusic(item, index)"
						v-if="type === 'local' || playListData.creator.userId === $Api.User.UserId"
					/>
				</span>
			</li>
		</DynamicScroller>
		<NoData v-show="listData.length === 0 && !loading" />
