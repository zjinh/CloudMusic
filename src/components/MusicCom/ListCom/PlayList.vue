	...
	<div class="cm-playlist-container">
		<ul :class="'cm-playlist-' + type" @scroll="scrollToLoad">
			<li v-for="(item, index) in data" :key="index" @click="clickToSelect(item)" ripple="">
				<img v-lazy="item.coverImgUrl || item.picUrl" src="" alt="" draggable="false" />
				<span>{{ item.name }}</span>
...
.cm-playlist-block li .count,
.cm-playlist-block li .creator {
	display: none;
}
...
.cm-playlist-list li:hover,
.cm-playlist-block li:hover,
.cm-playlist-recommend li:hover {
...
.cm-playlist-recommend .play_count,
.cm-playlist-recommend .count,
.cm-playlist-recommend .creator {
	display: none;
}
</style>
