  ...
  <div class="cm-videolist-container">
    <div class="cm-videolist-list" @scroll="scrollToLoad">
      <div
        ...
        :class="type ? 'video-self-block' : 'video-block'"
        ripple=""
      >
        ...
        >
        <CountTag class="count sf-icon-video">
          {{ $numberCount(item.playTime || item.playCount || 0) }}</CountTag
        >
        <CountTag class="time">
          {{ timeDeal(item.durationms || item.duration || 0) }}</CountTag
        >
      </div>
    </div>
