<template>
	<section class="cm-left">
		<div class="cm-left-head">
			<img draggable="false" class="logo" src="../../assets/img/logo/logo.png" alt="" />
			<div class="logo-text"></div>
			<p>愿每个人都被时光温柔以待</p>
		</div>
		<div class="cm-left-menu">
			<div class="cm-left-menu-container" v-for="(item, index) in MenuData" :key="index">
				<p>{{ item.name }}</p>
				<ul>
					<li v-for="(menu, m_index) in item.children" :key="m_index" ripple="" :class="menu.active" @click="change(index, m_index)">
...
.cm-left-head .logo-text {
	...
	margin-top: -3px;
	height: 26px;
	background: url('../../assets/img/logo/c-music.png');
...
.full .logo-text {
	...
	margin-top: -2px;
	margin-left: -3px;
	background: url('../../assets/img/logo/c-music-white.png');
}
.full p {
	color: #fff;
}
