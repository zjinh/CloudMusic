<template>
  <ul class="cd-mouse-menu" ref="MouseMenu" v-show="MouseMenuShow">
    <li
      ...
      :key="index"
      @click="MenuClick(item.commend, item.data)"
      v-show="toBoolean(item.vif, 'vif')"
    >
...
export default {
  ...
  methods: {
    ...
    MenuShow(flag) {
      ...
      menu.style.left =
        event.pageX +
        -parseFloat(createNode.getBoundingClientRect().left) +
        createNode.offsetLeft +
        "px";
      menu.style.top =
        event.pageY +
        -parseFloat(createNode.getBoundingClientRect().top) +
        createNode.offsetTop +
        "px";
      if (
        menu.getBoundingClientRect().left + menu.offsetWidth >
        createNode.getBoundingClientRect().width
      ) {
        menu.style.left =
          createNode.getBoundingClientRect().width -
      ...
      if (
        menu.getBoundingClientRect().top +
          menu.offsetHeight -
          createNode.getBoundingClientRect().top >
        createNode.offsetHeight
      ) {
        menu.style.top =
          createNode.getBoundingClientRect().height -
