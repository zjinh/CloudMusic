  ...
  <section class="cm-full-music-player animated slide-down-enter">
    <div class="cm-full-music-container" id="cm-full-player" @scroll="loadMore">
      <div class="cm-full-music-control-btn">
        <button
          tabindex="-1"
          class="sf-icon-ellipsis-h"
          @click="playerControl('')"
        ></button>
        <button
          tabindex="-1"
          class="sf-icon-dot-circle"
          @click="switchMode"
        ></button>
        <h1 v-show="!mode">
          {{ NowPlay.title }}
      ...
      <div class="cm-full-music-player-container">
        ...
        <div class="cm-full-poster-container">
          ...
          <div class="cm-full-music-poster">
            ...
            <button
              :class="playState + ' animated fadeIn'"
              v-show="!mode"
        ...
        <div class="cm-full-next-container">
          ...
          <button
            tabindex="-1"
            class="sf-icon-angle-right"
            @click="playerControl('next')"
          ></button>
        </div>
        <div
          class="cm-full-music-lrc"
          :style="{ width: mode ? 'calc(100% - 430px)' : '0' }"
        >
          <h1>{{ NowPlay.title }}</h1>
          <p class="artist">
            {{ NowPlay.artist }}
      ...
      <div
        class="cm-full-right-container"
        v-show="mode && NowPlay.type !== 'radio'"
      >
        <div class="cm-full-music-more-head">
          相似歌曲
        </div>
        <ul class="cm-full-simi-music">
          <li
            v-for="(item, index) in simiMusicList"
            :key="index"
            @click="playMusic(item, simiMusicList)"
          >
