	...
	<section class="cm-full-music-player animated slide-down-enter">
		<div class="cm-full-music-container" id="cm-full-player" @scroll="loadMore">
			<div class="cm-full-music-control-btn">
				<button tabindex="-1" class="sf-icon-ellipsis-h" @click="playerControl('')" />
				<button tabindex="-1" class="sf-icon-dot-circle" @click="switchMode" />
				<h1 v-show="!mode">
					{{ NowPlay.title }}
				</h1>
				<p v-show="!mode">
			...
			<div class="cm-full-music-player-container">
				<div class="cm-full-prev-container" v-show="!mode">
					<div class="line"></div>
					<button tabindex="-1" class="sf-icon-angle-left" @click="playerControl('prev')" />
				</div>
				<div class="cm-full-poster-container">
					<canvas id="wrap" height="300" width="300" />
					<div class="cm-full-music-poster">
						<img v-lazy="NowPlay.picture" src="" alt="" draggable="false" />
						<button :class="playState + ' animated fadeIn'" v-show="!mode" @click="playerControl('play')" />
					</div>
					<div class="cm-full-music-button" v-show="mode">
						<LikeMusic :music="NowPlay" />
						<AddMusic icon="sf-icon-star-o" :music="NowPlay" />
						<DownMusic :music="NowPlay" />
					</div>
					<p id="cm-fully-music-line-lrc" class="animated slideInDown" v-show="!mode" />
				</div>
				<div class="cm-full-next-container">
					<div class="line"></div>
					<button tabindex="-1" class="sf-icon-angle-right" @click="playerControl('next')" />
				</div>
			...
			<CommentList
				class="cm-full-player-comment"
				:_id="NowPlay.id"
				:data="commentData"
				:type="commentType"
				:style="{ width: NowPlay.type !== 'radio' ? '70%' : '100%' }"
				v-show="mode"
			>
			...
			<div class="cm-full-right-container" v-show="mode && NowPlay.type !== 'radio'">
				...
				<ul class="cm-full-simi-music">
					<li v-for="(item, index) in simiMusicList" :key="index" @click="playMusic(item, simiMusicList)">
				...
				<PlayList :data="simiPlayList" type="block" />
				<div class="cm-full-music-more-head">
					最近听过的用户
				</div>
				<ul class="cm-full-simi-user">
					<li v-for="(item, index) in simiUser" :key="index" @click="goUserDetail(item)">
						<img v-lazy="item.avatarUrl" src="" alt="" />
		...
		<BlurBackground :url="NowPlay.picture" style="height: 100%" />
	</section>
</template>
...
.cm-full-simi-music li,
.cm-full-simi-user li {
	width: 100%;
	display: flex;
	align-items: center;
	height: 50px;
...
.cm-full-simi-music li span,
.cm-full-simi-user li span {
	padding-left: 10px;
	color: #fff;
	font-size: 14px;
}
...
.cm-full-simi-music li:hover,
.cm-full-simi-user li:hover {
	background: rgba(255, 255, 255, 0.1);
}
.cm-full-simi-music li img,
