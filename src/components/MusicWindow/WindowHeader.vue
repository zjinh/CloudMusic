<template>
  <div
    ...
    :style="{
      'margin-top':
        data.resize && ButtonState === 'sf-icon-window-maximize' ? '2px' : '0',
      color: data.color ? data.color : '#333',
      background: data.background ? data.background : ''
    }"
  >
    <p v-show="data.head !== undefined ? data.head : true">
      {{ data.title }}
    </p>
    <ul class="window-actions">
      <li
        class="sf-icon-minus"
        v-show="data.mini === undefined ? true : data.mini"
        @click="mini"
      ></li>
      <li
        :class="ButtonState"
        v-show="data.resize === undefined ? true : data.resize"
        @click="restore"
      ></li>
