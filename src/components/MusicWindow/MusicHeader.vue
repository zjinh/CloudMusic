      ...
      <div class="cm-right-search-bubble-content">
        ...
        <div class="songs">
          ...
          <ul v-if="SearchSuggestResult.songs" class="animated slideInLeft">
            <li
              v-for="(item, index) in SearchSuggestResult.songs"
              :key="index"
              @click="playSearch(item)"
            >
              {{ item.name }}
            </li>
          </ul>
        ...
        <div class="album">
          ...
          <ul v-if="SearchSuggestResult.albums">
            <li
              v-for="(item, index) in SearchSuggestResult.albums"
              :key="index"
              @click="searchAlbum(item)"
              class="animated slideInLeft"
            >
      ...
      a = setTimeout(() => {
        ...
        if (this.SearchSuggest !== 0 && this.SearchKey.length) {
          this.$Api.Music.searchSuggest(this.SearchKey, rs => {
            ...
            this.SearchSuggestResult.songs = this.$handleListData(
              this.SearchSuggestResult.songs,
              "searchSuggest"
            );
