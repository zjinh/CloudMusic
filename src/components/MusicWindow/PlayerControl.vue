<template>
	<section class="cm-music-player">
		...
		<ul class="cm-play-mode">
			<li :class="'sf-icon-random ' + (playMethod === 'random' ? 'active' : '')" @click="changePlayMethod('random')" ripple="" title="随机播放" />
			<li :class="'sf-icon-repeat ' + (playMethod === 'repeat' ? 'active' : '')" @click="changePlayMethod('repeat')" ripple="" title="单曲循环" />
			<li :class="'sf-icon-list ' + (playMethod === 'list' ? 'active' : '')" @click="changePlayMethod('list')" ripple="" title="顺序播放" />
		</ul>
		<canvas width="600" height="240" id="canvas" />
		<audio
			ref="audio"
			id="audio"
...
import lyric from '../../tools/music/lyric';
