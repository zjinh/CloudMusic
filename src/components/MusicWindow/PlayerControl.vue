<template>
  <section class="cm-music-player">
    ...
    <ul class="cm-play-mode">
      <li
        :class="'sf-icon-random ' + (playMethod === 'random' ? 'active' : '')"
...
export default {
  ...
  created() {
    ...
    this.playMethod =
      localStorage.playMethod === undefined ? "list" : localStorage.playMethod;
  },
  methods: {
        ...
        switch (item.type || this.NowPlay.type) {
          case "online":
            this.$Api.Music.song.detail(
              this.NowPlay.id,
              rs => {
                this.PlayList[this.NowPlay.count].picture =
                  rs || this.$defaultAlbum;
        ...
        data => {
          if (data.pivot % 2 === 1) {
            this.$ipc.send("player-control", "lrc", {
              ...
              now: "left",
              right: data.target[data.pivot + 1]
                ? data.target[data.pivot + 1].innerHTML
                : ""
            });
          } else {
            this.$ipc.send("player-control", "lrc", {
              left: data.target[data.pivot + 1]
                ? data.target[data.pivot + 1].innerHTML
                : "",
              now: "right",
