  ...
  <div class="cm-page-main" @scroll="loadMore">
    <DetailPageHead :data="albumData" type="album">
      <div class="cm-album-content">
        <p class="album-desp">
          {{ albumData.publishTime ? "" : "收藏" }}时间: {{ albumData._time }}
        </p>
        ...
        <ButtonArea class="album-control">
          <button
            @click="subscribe"
            :class="subscribed ? 'sf-icon-star' : 'sf-icon-star-o'"
          >
            {{ subscribed ? " 已" : " " }}收藏
          </button>
    ...
    <div class="cm-album-detail-main">
      <SongList
        ...
        :data="albumDetail.musicList"
        :loading="loading"
      ></SongList>
      <CommentList
        v-show="nowType.type === 'comment'"
        :data="albumDetail.comment"
        type="album"
        :loading="loading"
      ></CommentList>
  ...
  methods: {
    init() {
      ...
      if (this.$route.query.data) {
        ...
        this.albumData._time = new Date(
          this.albumData.publishTime || this.albumData.subTime
        ).format("yyyy-MM-dd");
        let artist_text =
          (this.albumData.artist && this.albumData.artist.name) || "";
        if (this.albumData.artists && !artist_text) {
          this.albumData.artists.forEach((item, index) => {
            artist_text =
              artist_text +
              item.name +
              (index !== this.albumData.artists.length - 1 ? "/" : "");
          });
        } else {
