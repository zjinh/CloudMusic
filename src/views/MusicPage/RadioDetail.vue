	...
	<div class="cm-page-main" @scroll="loadMore">
		<DetailPageHead :data="radioData" type="radio">
			<div class="cm-radio-content">
				...
				<p class="radio-desp">节&nbsp;&nbsp;目&nbsp;数: {{ radioData.programCount }}</p>
				<ButtonArea class="radio-control">
					<button @click="subscribe" :class="subscribed ? 'sf-icon-star' : 'sf-icon-star-o'">
						{{ subscribed ? ' 已' : ' ' }}订阅({{ $numberCount(radioData.subCount) }})
					</button>
				</ButtonArea>
			</div>
			<div class="cm-radio-content">
				<p class="radio-desp">
					<span>{{ radioData.category }}</span
					>{{ radioData.desc }}
				</p>
			</div>
		</DetailPageHead>
		...
		<div class="cm-radio-detail-main">
			<RadioItem v-show="nowType.type === 'programs'" :data="radioDetail.programs" :loading="loading" />
		</div>
		<BackToTop />
	</div>
</template>
...
<script>
import RadioItem from '../../components/MusicCom/ListCom/RadioItem';
export default {
	name: 'RadioDetail',
	components: {
		RadioItem
