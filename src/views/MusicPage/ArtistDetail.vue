<template>
	<div class="cm-page-main" id="cm-artist-detail" @scroll="loadMore">
		...
		<TabBar :data="artistDataType" align="left" @select="tabBarChange" />
		<div class="cm-artist-detail-main">
			<SongList v-show="nowType.type === 'music'" :data="artistDetail.music" :loading="loading" />
			<VideoList v-show="nowType.type === 'mv'" :data="artistDetail.mv" :loading="loading" />
			<ArtistList v-show="nowType.type === 'simi'" :data="artistDetail.simi" @callback="init" :loading="loading" />
		</div>
		<BackToTop />
	</div>
</template>
