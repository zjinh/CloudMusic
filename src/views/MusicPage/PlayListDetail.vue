  ...
  <div class="cm-page-main" @scroll="loadMore">
    <DetailPageHead :data="playlistData" type="playlist">
      <div class="cm-playlist-content-50">
        ...
        <p class="playlist-desp">
          播放数:{{ $numberCount(playlistData.playCount) }}次
        </p>
        ...
        <ButtonArea class="playlist-control">
          ...
          <button
            ...
            :class="
              playlistData.subscribed ? 'sf-icon-heart' : 'sf-icon-heart-o'
            "
          >
            {{ playlistData.subscribed ? " 已" : " " }}收藏({{
              $numberCount(
                playlistData.bookCount || playlistData.subscribedCount
              )
            }})
          </button>
    ...
    <div class="cm-playlist-detail-main">
      <SongList
        ref="songList"
        v-show="nowType.type === 'musicList'"
        :data="playListDetail.musicList"
        :loading="loading"
        @remove="removeCallback"
      ></SongList>
      <CommentList
        v-show="nowType.type === 'comment'"
        :data="playListDetail.comment"
        type="playlist"
        :loading="loading"
      ></CommentList>
      <UserList
        v-show="nowType.type === 'subscribers'"
        :data="playListDetail.subscribers"
      ></UserList>
    </div>
    <BackToTop></BackToTop>
  </div>
...
export default {
  ...
  methods: {
    init() {
      ...
      if (this.$route.query.data || this.$route.query.list) {
        this.playlistData = JSON.parse(
          this.$route.query.data || this.$route.query.list
        );
      }
    ...
    playAll() {
      this.$refs.songList.clickToPlay(
        this.$refs.songList.listData[0],
        this.$refs.songList.listData
      );
    },
    downloadList() {
