		...
		<DetailPageHead :data="playlistData" type="playlist">
			<div class="cm-playlist-content-50">
				...
				<div class="playlist-author">
					<img v-lazy="playlistData.creator.avatarUrl" src="" alt="" />
					<span>{{ playlistData.creator.nickname }}</span>
				</div>
				<ButtonArea class="playlist-control">
					<button class="sf-icon-play-circle" @click="playAll">
						播放全部
					</button>
					<button
						@click="subscribe"
						:disabled="this.$Api.User.UserId === playlistData.creator.userId"
						:class="playlistData.subscribed ? 'sf-icon-heart' : 'sf-icon-heart-o'"
					>
						{{ playlistData.subscribed ? ' 已' : ' ' }}收藏({{ $numberCount(playlistData.bookCount || playlistData.subscribedCount) }})
					</button>
			...
			<div class="cm-playlist-content-50 playlist-desp2">简介：{{ playlistData.description || '暂无简介' }}</div>
		</DetailPageHead>
		<TabBar :data="playlistDataType" align="left" @select="tabBarChange" />
		<div class="cm-playlist-detail-main">
			<SongList ref="songList" v-show="nowType.type === 'musicList'" :data="playListDetail.musicList" :loading="loading" @remove="removeCallback" />
			<CommentList v-show="nowType.type === 'comment'" :data="playListDetail.comment" type="playlist" :loading="loading" />
			<UserList v-show="nowType.type === 'subscribers'" :data="playListDetail.subscribers" />
		</div>
		<BackToTop />
	</div>
</template>
