<template>
	<div class="cm-page-main" id="cm-user-detail" @scroll="loadMore">
		...
		<TabBar :data="userDataType" align="left" @select="tabBarChange" />
		<div class="cm-user-detail-main">
			<PlayList v-show="nowType.type === 'playList'" :data="userDetail.playList" :loading="loading" />
			<UserList v-show="nowType.type === 'follows'" :data="userDetail.follows" :loading="loading" @callback="init" />
			<UserList v-show="nowType.type === 'followeds'" :data="userDetail.followeds" :loading="loading" @callback="init" />
			<RadioList v-show="nowType.type === 'audio'" :data="userDetail.audio" :loading="loading" />
		</div>
		<BackToTop />
	</div>
</template>
		...
		getUserDetail() {
			this.$Api.User.detail(this.userData.userId, rs => {
				for (let i in rs.profile) {
					if (rs.profile.hasOwnProperty(i)) {
						this.$nextTick(() => {
