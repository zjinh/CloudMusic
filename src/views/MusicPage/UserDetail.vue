<template>
  <div class="cm-page-main" id="cm-user-detail" @scroll="loadMore">
    <DetailPageHead :data="userData" type="user">
      <div class="cm-user-content">
        ...
        <ButtonArea>
          <button
            @click="subscribe"
            :class="userData.followed ? 'sf-icon-heart' : 'sf-icon-heart-o'"
          >
            {{ userData.followed ? " 已" : " " }}关注
          </button>
        </ButtonArea>
      </div>
      <div class="cm-user-content">
        <p class="signature">
          个人介绍：{{ userData.signature || "暂无介绍" }}
        </p>
      </div>
    </DetailPageHead>
    ...
    <div class="cm-user-detail-main">
      <PlayList
        v-show="nowType.type === 'playList'"
        :data="userDetail.playList"
        :loading="loading"
      ></PlayList>
      <UserList
        v-show="nowType.type === 'follows'"
        :data="userDetail.follows"
        :loading="loading"
        @callback="init"
      ></UserList>
      <UserList
        v-show="nowType.type === 'followeds'"
        :data="userDetail.followeds"
        :loading="loading"
        @callback="init"
      ></UserList>
      <RadioList
        v-show="nowType.type === 'audio'"
        :data="userDetail.audio"
        :loading="loading"
      ></RadioList>
    </div>
    <BackToTop></BackToTop>
