<template>
	<div class="cm-page-main">
		<div class="cm-discover-head">
			...
			<div class="container">
				<button :class="'sf-icon-angle-' + (showType ? 'right' : 'left')" @click="switchType" />
				<p class="tips" v-if="showType">
					推荐歌曲
				</p>
				<p class="tips" v-else>
					推荐歌单
				...
				<p class="right-tips">
					<i class="sf-icon-lightbulb" />
					<span>根据听过的音乐，每天自动生成</span>
				</p>
			</div>
		</div>
		<SongList v-show="showType" :data="RecommendSong" :loading="loading" />
		<PlayList v-show="!showType" :data="RecommendPlayList" :loading="loading2" type="recommend" />
	</div>
</template>
